...
- どんどん伸びる `circleci.yml`
  - `circleci.yml` は CI だから基本走る（走らせたくない部分は filter する）
  - workflowごとに `require` をたどっていけばいい
  - ↑は可視化ツールのおかげです。
...
- 先述の重いサービスの別サービス化のプロセスを、ATLASチームのGAE本に書いた。
- `「既存の動いているサービスを切り出す方法」` としてまとめられたのが良かった。
